CovariateTest_mod <- function(species.table = NULL, 
                              genus.table = NULL, 
                              family.table = NULL,
                              order.table = NULL, 
                              class.table = NULL, 
...
if(Sys.info()[['sysname']] == "Windows") {
  ...
  if (class(meta) == "phyloseq") {
    metadata <- data.frame(meta@sam_data)
  } else {
...
if(ncol(taxa)>0) {
  ...
  if (length(select.by) != 0) {
    ...
    if (select %in% metadata$selection) {
      ...
      taxa <- taxa %>%
        dplyr::filter(rownames(.) %in% rownames(metadata))
    } else {
  ...
  taxa[taxa<0]<-0
  tidyvar <- sym(covariate)
  dataset <- data.frame(metadata, taxa) %>%
    dplyr::mutate(!!tidyvar := as.numeric(as.character(!!tidyvar)))
  ...
  if (length(group) != 0) {
    ...
    if (length(subject.ID) != 0) {
      ...
      modeldata <- na.omit(dataset[, c("group", group,names(taxa), paste(confounders, sep = ", "),
                                       "ID", covariate, "ReadCount")[c("group",group, names(taxa),paste(confounders, sep = ", "), 
          ...
          for (i in names(which(colSums(modeldata[modeldata$group==j,names(taxa)]>0)>6))) { 
            ...
            if(length(model[[paste(i,j)]])==0){
              if(abs(mean(modeldata[modeldata$group==j,i]/modeldata$ReadCount[modeldata$group==j])-median(modeldata[modeldata$group==j,i]/modeldata$ReadCount[modeldata$group==j]))/median(modeldata[modeldata$group==j,i]/modeldata$ReadCount[modeldata$group==j])<0.25){
                model[[paste(i,j)]] <- tryCatch(nlme::lme(as.formula(paste("((", i, ")/ReadCount)~", confounders_formula, "+ ",covariate)),random = ~1 | ID,  data = modeldata[modeldata$group==j,]),
                                                error = function(e) NULL)
                
              } else{
                model[[paste(i,j)]] <- tryCatch(nlme::lme(as.formula(paste("log((", i, "+1)/ReadCount)~", confounders_formula, "+ ",covariate)),random = ~1 | ID,  data = modeldata[modeldata$group==j,]),
                                                error = function(e) NULL)
              }}  
            ...
            if(nonzero){
              ...
              if(length(model[[paste(i,j,"nonzero")]])==0){
                if(abs(mean(modeldata[modeldata$group==j&modeldata[,i]>0,i]/modeldata$ReadCount[modeldata$group==j&modeldata[,i]>0])-median(modeldata[modeldata$group==j&modeldata[,i]>0,i]/modeldata$ReadCount[modeldata$group==j&modeldata[,i]>0]))/median(modeldata[modeldata$group==j&modeldata[,i]>0,i]/modeldata$ReadCount[modeldata$group==j&modeldata[,i]>0])<0.25){
                  model[[paste(i,j,"nonzero")]] <- tryCatch(nlme::lme(as.formula(paste("((", i, ")/ReadCount)~", confounders_formula, "+ ",covariate)),random = ~1 | ID,  data = modeldata[modeldata$group==j&modeldata[,i]>0,]),
                                                            error = function(e) NULL)
                  
                } else{
                  model[[paste(i,j,"nonzero")]] <- tryCatch(nlme::lme(as.formula(paste("log((", i, "+1)/ReadCount)~", confounders_formula, "+ ",covariate)),random = ~1 | ID,  data = modeldata[modeldata$group==j&modeldata[,i]>0,]),
                                                            error = function(e) NULL)
                }}  
      ...
      modeldata <- na.omit(dataset[ , c("group",group, names(taxa), paste(confounders, sep = ", "),  covariate, "ReadCount")[c("group",group, names(taxa), paste(confounders, sep = ", "), 
                                                                                   covariate, "ReadCount") != ""]])
      modeldata[,group]<-modeldata[,group][drop=T]
      ...
      for (j in levels(dataset$group)) {      
        ...
        for (i in names(which(colSums(modeldata[modeldata$group==j,names(taxa)]>0)>6))){
          ...
          if(length(model[[paste(i,j)]])==0){
            if(abs(mean(modeldata[modeldata$group==j,i]/modeldata$ReadCount[modeldata$group==j])-median(modeldata[modeldata$group==j,i]/modeldata$ReadCount[modeldata$group==j]))/median(modeldata[modeldata$group==j,i]/modeldata$ReadCount[modeldata$group==j])<0.25){
              model[[paste(i,j)]] <- tryCatch(lm(as.formula(paste("((", i, ")/ReadCount)~", confounders_formula, "+ ",covariate)),  data = modeldata[modeldata$group==j,]),
                                              error = function(e) NULL)
              
            } else {
              model[[paste(i,j)]] <- tryCatch(lm(as.formula(paste("log((", i, "+1)/ReadCount)~", confounders_formula, "+ ",covariate)),  data = modeldata[modeldata$group==j,]),
                                              error = function(e) NULL)
            }}  
          ...
          if(nonzero){
            ...
            if(length(model[[paste(i,j,"nonzero")]])==0){
              if(abs(mean(modeldata[modeldata$group==j&modeldata[,i]>0,i]/modeldata$ReadCount[modeldata$group==j&modeldata[,i]>0])-median(modeldata[modeldata$group==j&modeldata[,i]>0,i]/modeldata$ReadCount[modeldata$group==j&modeldata[,i]>0]))/median(modeldata[modeldata$group==j&modeldata[,i]>0,i]/modeldata$ReadCount[modeldata$group==j&modeldata[,i]>0])<0.25){
                model[[paste(i,j,"nonzero")]] <- tryCatch(lm(as.formula(paste("((", i, ")/ReadCount)~", confounders_formula, "+ ",covariate)), data = modeldata[modeldata$group==j&modeldata[,i]>0,]),
                                                          error = function(e) NULL)
                
              } else{
                model[[paste(i,j,"nonzero")]] <- tryCatch(lm(as.formula(paste("log((", i, "+1)/ReadCount)~", confounders_formula, "+ ",covariate)), data = modeldata[modeldata$group==j&modeldata[,i]>0,]),
                                                          error = function(e) NULL)
              }}  
    ...
    sig <- covariate_test %>%
      ...
      dplyr::filter(if_any(everything(), ~. < p.cutoff)) %>%
      rownames()    
    
    if(nonzero) {
      sigNonzero <- covariate_test %>%
        dplyr::select(ends_with("nonzero_p")) %>%
        dplyr::filter(if_any(everything(), ~. < p.cutoff)) %>%
        rownames()  
      if(length(sigNonzero) == 0) {
        sigNonzero <- NULL
      }
    } else {  
      sigNonzero <-  NULL
    ...
    covariate_test <- covariate_test[order(covariate_test$taxon),] %>%
      dplyr::mutate(across(.cols = contains("model"), .fns = ~tidyr::replace_na(.x, "None")))
    
    writexl::write_xlsx(covariate_test, path = 
                  here::here(
                    "Results", "CovariateTest", dir_for_res, 
                    paste(
                      "CovariateTest_", covariate, "_", group, 
                      "_", select.by, "_", select, "_prev_", min.prevalence*100, 
    ...
    if (length(sig) > 0) {
      ...
      if(length(confounders[confounders!=""])>0){
        ...
        # Add Sample names as a column to facilitate "row_update"
        resids <- resids  %>%
        ...
        for (j in levels(modeldata$group)) {
          ...
          for(i in setdiff(sig,sigNonzero)){
            resids <-  tryCatch(
              ...
              # Resids update model
              resid(update(model[[paste(i,j)]],as.formula(paste(".~. -",covariate,sep="")))) %>%
                
                # Cleaning and conversion
                as.data.frame() %>%
                purrr::set_names(i) %>%
                dplyr::mutate("group" = j) %>%
                tibble::rownames_to_column(var = "Sample_names") %>%
                
                # Add newly computed resids to specific columns
                dplyr::rows_update(resids, .,  by = c("group", "Sample_names")),
              error = function(e) resids)
      ...
      if(pdf){
        ggsave(p, width = 15, height = 10,
               path = here::here("Results", "CovariateTest", dir_for_res), 
               filename = paste("CovariatePlot_", covariate,
                                "_", group, "_",
                                select.by, "_", select, "_",
                                "prev_", min.prevalence*100,
                                "_abd_", min.abundance*100,
                                ".pdf", sep = ""), 
               device = "pdf");
        plot(p)
    ...
    if (length(subject.ID) != 0) {
      dataset$ID <- as.factor(dataset[, subject.ID])
      modeldata <- na.omit(dataset[, c(names(taxa), paste(confounders, sep = ", "),  "ID", 
                                       covariate, "ReadCount")[c(names(taxa), paste(confounders, sep = ", "),  "ID", 
                                                                 covariate, "ReadCount") != ""]])
      ...
      for (i in names(colSums(modeldata[,names(taxa)]>0)[colSums(modeldata[,names(taxa)]>0)>6])) { 
        ...
        if(length(model[[i]])==0){
          if(abs(mean(modeldata[,i]/modeldata$ReadCount)-median(modeldata[,i]/modeldata$ReadCount))/median(modeldata[,i]/modeldata$ReadCount)<0.25){
            model[[i]] <- tryCatch(nlme::lme(as.formula(paste("((", i, ")/ReadCount)~", confounders_formula, "+ ",covariate)),random = ~1 | ID,  data = modeldata),
                                   error = function(e) NULL)
            
          } else{
            model[[i]] <- tryCatch(nlme::lme(as.formula(paste("log((", i, "+1)/ReadCount)~", confounders_formula, "+ ",covariate)),random = ~1 | ID,  data = modeldata),
                                   error = function(e) NULL)
          }}  
        ...
        if(nonzero){
          ...
          if(length(model[[paste(i,"nonzero")]])==0){
            if(abs(mean(modeldata[modeldata[,i]>0,i]/modeldata$ReadCount[modeldata[,i]>0])-median(modeldata[modeldata[,i]>0,i]/modeldata$ReadCount[modeldata[,i]>0]))/median(modeldata[modeldata[,i]>0,i]/modeldata$ReadCount[modeldata[,i]>0])<0.25){
              model[[paste(i,"nonzero")]] <- tryCatch(nlme::lme(as.formula(paste("((", i, ")/ReadCount)~", confounders_formula, "+ ",covariate)),random = ~1 | ID,  data = modeldata[modeldata[,i]>0,]),
                                                      error = function(e) NULL)
              
            } else{
              model[[paste(i,"nonzero")]] <- tryCatch(nlme::lme(as.formula(paste("log((", i, "+1)/ReadCount)~", confounders_formula, "+ ",covariate)),random = ~1 | ID,  data = modeldata[modeldata[,i]>0,]),
                                                      error = function(e) NULL)
            }}  
      ...
      modeldata <- na.omit(dataset[, c(names(taxa), paste(confounders, sep = ", "), covariate, "ReadCount")[c(names(taxa),paste(confounders, sep = ", "), 
                                                                                                 covariate, "ReadCount") != ""]])  
      ...
      for (i in names(colSums(modeldata[,names(taxa)]>0)[colSums(modeldata[,names(taxa)]>0)>6])){
        ...
        if(length(model[[i]])==0){
          if(abs(mean(modeldata[,i]/modeldata$ReadCount)-median(modeldata[,i]/modeldata$ReadCount))/median(modeldata[,i]/modeldata$ReadCount)<0.25){
            model[[i]] <- tryCatch(lm(as.formula(paste("((", i, ")/ReadCount)~", confounders_formula, "+ ",covariate)),  data = modeldata),
                                   error = function(e) NULL)
            
          } else {
            model[[i]] <- tryCatch(lm(as.formula(paste("log((", i, "+1)/ReadCount)~", confounders_formula, "+ ",covariate)),  data = modeldata),
                                   error = function(e) NULL)
          }}  
        ...
        if(nonzero){
          ...
          if(length(model[[paste(i,"nonzero")]])==0){
            if(abs(mean(modeldata[modeldata[,i]>0,i]/modeldata$ReadCount[modeldata[,i]>0])-median(modeldata[modeldata[,i]>0,i]/modeldata$ReadCount[modeldata[,i]>0]))/median(modeldata[modeldata[,i]>0,i]/modeldata$ReadCount[modeldata[,i]>0])<0.25){
              model[[paste(i,"nonzero")]] <- tryCatch(lm(as.formula(paste("((", i, ")/ReadCount)~", confounders_formula, "+ ",covariate)), data = modeldata[modeldata[,i]>0,]),
                                                      error = function(e) NULL)
              
            } else {
              model[[paste(i,"nonzero")]] <- tryCatch(lm(as.formula(paste("log((", i, "+1)/ReadCount)~", confounders_formula, "+ ",covariate)), data = modeldata[modeldata[,i]>0,]),
                                                      error = function(e) NULL)
            }}  
