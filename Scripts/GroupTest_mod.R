GroupTest_mod <- function(species.table=NULL, genus.table=NULL, family.table=NULL, order.table=NULL,
                          class.table=NULL, phylum.table=NULL, meta, group,
                          group_name = NULL, compare.to = NULL, add_rwns = NULL, 
                          dir_for_res = NULL, readcount.cutoff = 0, confounders = NULL, subject.ID = NULL,
                          outlier.cutoff = 3, p.cutoff = 0.05, select.by = NULL, select = NULL, pdf = T,
                          min.prevalence = 0.05, min.abundance = 0, label.direction = 1, keep.result = F,
                          nonzero = T, relative = T, show_quartz = F) {
  ...
  if(ncol(taxa)>0) {   
    ...
    if(nrow(na.omit(group_test))>0){
      ...
      GT_plot <- dataset2 %>%
        dplyr::select(all_of(c(sig, group)))
